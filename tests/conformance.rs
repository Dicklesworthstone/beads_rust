...
fn conformance_close_updates_dependents() {
    ...
    // B should now be unblocked (no longer appears in the blocked list)
    let br_blocked_after = workspace.run_br(["blocked", "--json"], "blocked_after");
    let bd_blocked_after = workspace.run_bd(["blocked", "--json"], "blocked_after");

    assert!(br_blocked_after.status.success(), "br blocked failed");
    assert!(bd_blocked_after.status.success(), "bd blocked failed");
