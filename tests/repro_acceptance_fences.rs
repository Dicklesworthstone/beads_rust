...
fn workspace() -> BrWorkspace {
    ...
    let init = run_br(&workspace, ["init", "--prefix", "fence"], "init_fence_workspace");
...
fn show(workspace: &BrWorkspace, id: &str, step: &str) -> Value {
    ...
    if let Some(rows) = value.as_array() {
        assert_eq!(rows.len(), 1, "exactly the requested issue must be returned");
        rows[0].clone()
    } else {
...
fn assert_refused(workspace: &BrWorkspace, args: &[&str], expected: &str, step: &str) {
    let run = run_br(workspace, args.iter().copied(), step);
    assert!(!run.status.success(), "{step} unexpectedly succeeded: {run:?}");
...
fn acceptance_fences_cli_edits_real_items_and_preserves_examples_and_export() {
    let _log = common::test_log("acceptance_fences_cli_edits_real_items_and_preserves_examples_and_export");
    let workspace = workspace();
    let body = "````markdown\r\n```rust\r\n- [ ] example only\r\n```\r\n````\r\n  * [\u{a0}] Réal requirement\r\n  * [X] Existing complete\r\n";
    let id = create(&workspace, "Real requirements", "--acceptance-criteria", body);
    let before = show(&workspace, &id, "show_fences_before_edit");
    assert_eq!(before["acceptance_criteria"], body);
    assert_eq!(before["acceptance_items"], json!([
    ...
    assert_refused(
        &workspace,
        &["update", &id, "--title", "Must not change", "--check-acceptance", "example only", "--json"],
    ...
    success_json(
        &workspace,
        &["update", &id, "--check-acceptance", "1", "--uncheck-acceptance", "2", "--add-acceptance", "New criterion", "--json"],
    ...
    assert_eq!(after["acceptance_items"], json!([
    ...
    assert_eq!(exported_issue(&workspace, &id, "export_real_edits")["acceptance_criteria"], expected);
}

#[test]
fn acceptance_fences_cli_refuses_unclosed_appends_without_partial_mutations() {
    let _log = common::test_log("acceptance_fences_cli_refuses_unclosed_appends_without_partial_mutations");
    let workspace = workspace();
    for (index, ending) in ["```", "````language", "~~~~"].into_iter().enumerate() {
        let body = format!("- [ ] Existing\n````markdown\n{ending}\n- [x] example only\n");
        let id = create(&workspace, &format!("Unclosed case {index}"), "--acceptance-criteria", &body);
        let before = show(&workspace, &id, "show_unclosed_before");
        let export_before = exported_issue(&workspace, &id, "export_unclosed_before");
        assert_refused(
            &workspace,
            &["update", &id, "--title", "Must not change", "--check-acceptance", "1", "--add-acceptance", "Must not disappear", "--json"],
            "unclosed code fence",
            "refuse_unclosed_append",
        );
        assert_eq!(show(&workspace, &id, "show_unclosed_after_refusal"), before);
        assert_eq!(exported_issue(&workspace, &id, "export_unclosed_after_refusal"), export_before);
        // A valid edit before the open fence remains usable, without --force.
        success_json(&workspace, &["update", &id, "--check-acceptance", "1", "--json"], "check_before_open_fence");
        assert_eq!(show(&workspace, &id, "show_valid_check")["acceptance_criteria"], body.replacen("[ ] Existing", "[x] Existing", 1));
    }
}

#[test]
fn prerequisite_fences_cli_refuses_false_completion_and_commits_real_completion() {
    let _log = common::test_log("prerequisite_fences_cli_refuses_false_completion_and_commits_real_completion");
    let workspace = workspace();
    configure_prerequisite_policy(&workspace);
    let id = create(&workspace, "Pending preparation", "--prerequisites", MISLEADING_CLOSE);
    ...
    assert_refused(
        &workspace,
        &["update", &id, "--status", "handoff", "--title", "Must not change", "--transition-comment", "Must not persist", "--json"],
        "transition_prerequisites_incomplete",
        "refuse_false_prerequisite_completion",
    );
    assert_eq!(show(&workspace, &id, "show_prerequisites_after_refusal"), before);
    assert_eq!(exported_issue(&workspace, &id, "export_prerequisites_after_refusal"), export_before);

    let complete = MISLEADING_CLOSE.replace("[ ] unfinished prerequisite", "[x] unfinished prerequisite");
    success_json(
        &workspace,
        &["update", &id, "--prerequisites", &complete, "--status", "handoff", "--transition-comment", "Real preparation verified", "--json"],
    ...
    let comments = exported["comments"].as_array().expect("committed transition comment");
...
fn prerequisite_fences_cli_rejects_example_only_evidence_but_not_unchecked_examples() {
    let _log = common::test_log("prerequisite_fences_cli_rejects_example_only_evidence_but_not_unchecked_examples");
    let workspace = workspace();
    configure_prerequisite_policy(&workspace);
    let only = create(&workspace, "Examples are not evidence", "--prerequisites", EXAMPLES_ONLY);
    let before = exported_issue(&workspace, &only, "export_examples_only_before");
    assert_refused(
        &workspace,
        &["update", &only, "--status", "handoff", "--transition-comment", "Must not persist", "--json"],
        "transition_prerequisites_incomplete",
        "refuse_examples_only",
    );
    assert_eq!(exported_issue(&workspace, &only, "export_examples_only_after"), before);

    let complete = format!("{}- [x] Real preparation\n", EXAMPLES_ONLY.replace("[x] example", "[ ] example"));
    let valid = create(&workspace, "Genuine completion", "--prerequisites", &complete);
    success_json(
        &workspace,
        &["update", &valid, "--status", "handoff", "--transition-comment", "Real preparation verified", "--json"],
