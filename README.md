...
`--merge`, `--status`, `--witness`, or `--reconcile-additive` so the data
direction and authority are explicit. `br sync --status` never probes Git; run
`br vcs-status --json` only when Git visibility is explicitly wanted.
