...
pub(crate) fn restore_tombstones(
    ...
    storage.with_write_transaction(|storage| {
        for tombstone in tombstones {
            storage.upsert_issue_for_import_in_tx(&tombstone.issue)?;
        }
        for tombstone in tombstones {
            if let Some(labels) = &tombstone.labels {
                storage.sync_labels_for_import_in_tx(&tombstone.issue.id, labels)?;
            }
            if let Some(dependencies) = &tombstone.dependencies {
                storage.sync_dependencies_for_import_in_tx(&tombstone.issue.id, dependencies)?;
            }
            if let Some(comments) = &tombstone.comments {
                storage.sync_comments_for_import_in_tx(&tombstone.issue.id, comments)?;
            }
            storage.replace_dirty_issue_marker_in_tx(&tombstone.issue.id, &marked_at)?;
        }
        Ok(())
    })?;
