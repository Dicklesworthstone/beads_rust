...
///
/// The countdown describes only this time gate. Even an `open` issue can have
/// unresolved dependencies or other readiness exclusions. Returns an empty
/// string once the gate has elapsed; the other readiness rules still apply.
fn defer_until_countdown_suffix(defer_until: DateTime<Utc>, now: DateTime<Utc>) -> String {
    let remaining = defer_until.signed_duration_since(now);
    if remaining <= chrono::Duration::zero() {
        return String::new();
    }
    // Preserve the sub-day distinction and label partial days explicitly:
    // 47 hours must not promise that the gate opens in just one day.
    ...
    if days > 0 && remaining > chrono::Duration::days(days) {
        format!(" (gate opens in over {when})")
    } else {
        format!(" (gate opens in {when})")
    }
}
...
fn format_issue_details(details: &IssueDetails, use_color: bool, wrap: bool) -> String {
    ...
    if let Some(defer) = &issue.defer_until {
        let suffix = defer_until_countdown_suffix(*defer, Utc::now());
        let _ = writeln!(
            output,
...
mod tests {
    ...
    #[test]
    fn defer_until_countdown_describes_only_the_time_gate() {
        let now = Utc.with_ymd_and_hms(2026, 9, 4, 12, 0, 0).unwrap();
        let in_seven_days = Utc.with_ymd_and_hms(2026, 9, 11, 12, 0, 0).unwrap();
        assert_eq!(
            defer_until_countdown_suffix(in_seven_days, now),
            " (gate opens in 7 days)"
        );
    }

    #[test]
    fn defer_until_countdown_does_not_claim_readiness_for_any_status() {
        let mut details = details_for_defer_gate(
        ...
        for status in [
            ...
            details.issue.status = status;
            let output = format_issue_details(&details, false, false);
            assert!(
                output.contains("(gate opens in over 7 days)") && !output.contains("(ready in"),
                "status {:?} must not claim readiness: {output}",
                details.issue.status
            );
        }
    }
    ...
    fn defer_until_countdown_singular_and_sub_day() {
        let now = Utc.with_ymd_and_hms(2026, 9, 4, 12, 0, 0).unwrap();
        assert_eq!(
            defer_until_countdown_suffix(Utc.with_ymd_and_hms(2026, 9, 5, 12, 0, 0).unwrap(), now),
            " (gate opens in 1 day)"
        );
        // 23h59m is still under a day: truncating, never rounding up to a day
        // that has not actually been waited out.
        assert_eq!(
            defer_until_countdown_suffix(Utc.with_ymd_and_hms(2026, 9, 5, 11, 59, 0).unwrap(), now),
            " (gate opens in under a day)"
        );
        assert_eq!(
            defer_until_countdown_suffix(Utc.with_ymd_and_hms(2026, 9, 4, 12, 0, 1).unwrap(), now),
            " (gate opens in under a day)"
        );
    }

    #[test]
    fn defer_until_countdown_distinguishes_partial_from_exact_days() {
        let now = Utc.with_ymd_and_hms(2026, 9, 4, 12, 0, 0).unwrap();
        for remaining in [
            chrono::Duration::days(1) + chrono::Duration::nanoseconds(1),
            chrono::Duration::hours(47),
        ] {
            assert_eq!(
                defer_until_countdown_suffix(now + remaining, now),
                " (gate opens in over 1 day)"
            );
        }
        assert_eq!(
            defer_until_countdown_suffix(now + chrono::Duration::days(2), now),
            " (gate opens in 2 days)"
        );
    }
    ...
    fn defer_until_countdown_is_empty_once_the_gate_elapsed() {
        ...
        assert_eq!(defer_until_countdown_suffix(now, now), String::new());
        assert_eq!(
            defer_until_countdown_suffix(Utc.with_ymd_and_hms(2026, 8, 1, 0, 0, 0).unwrap(), now),
            String::new()
        );
    }
    ...
    fn show_text_annotates_a_future_defer_gate() {
        ...
        let output = format_issue_details(&details, false, false);
        assert!(
            output.contains("Deferred until: ") && output.contains("(gate opens in over 7 days)"),
    ...
    fn show_text_leaves_an_elapsed_defer_gate_unannotated() {
        ...
        assert!(
            !output.contains("(ready in") && !output.contains("(gate opens in"),
