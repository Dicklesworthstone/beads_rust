...
use crate::sync::{
    ...
    MergeContext, OrphanMode, ReviewedAdditiveReconcilePlanRequest,
    ReviewedAdditiveReconcileRequest, SyncMergeIntent, SyncMergeNoteWitness, SyncMergePendingPhase,
    SyncMergePendingReceipt, analyze_jsonl_snapshot,
    apply_reviewed_additive_reconcile_under_authority, canonical_sync_path_sha256,
    capture_sync_database_witness, compute_jsonl_snapshot_content_hash, compute_staleness,
    ...
    load_base_snapshot_from_source, plan_reviewed_additive_reconcile,
...
pub fn validate_sync_mode_args(args: &SyncArgs) -> Result<()> {
    ...
    if mode_count > 1 {
        return Err(BeadsError::Validation {
            ...
            reason:
                "Must specify exactly one of --flush-only, --import-only, --merge, --status, --witness, or --reconcile-additive"
                    .to_string(),
        });
    }
    if mode_count == 0 {
        return Err(BeadsError::Validation {
            field: "mode".to_string(),
            reason:
                "Must specify one of --flush-only, --import-only, --merge, --status, --witness, or --reconcile-additive"
                    .to_string(),
        });
    }
...
fn maybe_delegate_rebuild(
    ...
    // tombstones against that same immutable generation, then imports the
    // identical bytes. Keeping this at the recovery boundary prevents a
    // path replacement from mixing filter state from A with rows from B.
...
fn replace_database_from_jsonl_snapshot(
    ...
    preserved_tombstones: &[crate::sync::PreservedTombstone],
) -> Result<ImportResult> {
    let startup = config::load_startup_config_with_paths(beads_dir, cli.db.as_ref())?;
...
mod tests {
    ...
    use crate::sync::{
        AdditiveReconcileConfig, ConflictResolution, PreservedTombstone,
        capture_jsonl_source_snapshot, plan_additive_reconcile, restore_tombstones,
