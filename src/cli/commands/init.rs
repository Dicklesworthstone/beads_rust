...
/// The canonical `.beads/.gitignore`.
///
/// Single source of truth: `br init` writes it, and `br doctor --repair`
/// writes this same content when the file is missing entirely, so the two
/// paths cannot disagree (GitHub #501). When the file exists but is merely
/// incomplete, doctor appends the individual patterns it maintains instead,
/// leaving an operator-authored file otherwise untouched.
pub(crate) const BEADS_GITIGNORE: &str = r"# Database
*.db
*.db-journal
*.db-shm
...
"
    ...
    let gitignore_created = if gitignore_existed {
        ...
        write_init_file_if_missing(&gitignore_path, BEADS_GITIGNORE.as_bytes())?
    };
